...
if (!empty($_POST['form_csvexport'])) {
    ...
    header("Content-Disposition: attachment; filename=patient_latest_encounters.csv");
...
<form name='theform' id='theform' method='post' action='patient_latest_encounters.php' onsubmit='return top.restoreSession()'>
...
if (!empty($_POST['form_refresh']) || !empty($_POST['form_csvexport'])) {
  ...
  <table class='table' id='mymaintable'>
   <thead class='thead-light'>
    ...
    <th> <?php echo xlt('Reason for encounter'); ?> </th>
   </thead>
 <tbody>
        <?php
    } // end not export
